...
public class DefaultJunjieRedisCache  implements JunjieRedisCache{
	 ...
	 * @param expiration
	 */
	DefaultJunjieRedisCache(String name, byte[] prefix, RedisTemplate<? extends Object, ? extends Object> template, long expiration) {
	...
	@Override
	public void put(final Object key,final Object value,final Long expirationSeconds) {
		final byte[] keyBytes = computeKey(key);
		final byte[] valueBytes = convertToBytesIfNecessary(template.getValueSerializer(), value);
		...
		template.execute(new RedisCallback<Object>() {
			public Object doInRedis(RedisConnection connection) throws DataAccessException {
				...
				if (expirationSeconds > 0) {
					connection.expire(keyBytes, expirationSeconds);
					// update the expiration of the set of keys as well
					connection.expire(setName, expirationSeconds);
				}
				connection.exec();
